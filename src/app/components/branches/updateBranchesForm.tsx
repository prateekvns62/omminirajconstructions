...
export default function UpdateServicesForm({ branch }: { branch: BranchType }) {
    const [fileNames, setFileNames] = useState<{ [key: string]: string }>({});
    const [isLoading, setIsLoading] = useState(false);
    const [formData, setFormData] = useState<BranchType>(branch);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`/api/branches/saveBranch/${branch.id}`, {
                method: "POST",
            ...
            <div className="flex w-full gap-x-8 px-4">
                ...
                <div className="w-[72%] p-10 py-8 bg-white">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <label className="block text-lg font-bold text-black">Branch Name <span className="text-red-500 text-2xl leading-none">*</span></label>
                        <input type="text" name="branchName" placeholder="Branch Name..." value={formData.branchName} onChange={handleChange} className="w-full p-2 border rounded" required />

                        <label className="block text-lg font-bold text-black">Branch Code <span className="text-red-500 text-2xl leading-none">*</span></label>
                        <input type="text" name="branchCode" placeholder="Branch Code..." value={formData.branchCode} onChange={handleChange} className="w-full p-2 border rounded" required />

                        <label className="block text-lg font-bold text-black">Location <span className="text-red-500 text-2xl leading-none">*</span></label>
                        ...
                        <label className="block text-lg font-bold text-black">Map Iframe <span className="text-red-500 text-2xl leading-none">*</span></label>
                        <textarea rows={5} name="mapIframe" placeholder="Map Iframe..." value={formData.mapIframe} onChange={handleChange} className="w-full p-2 border rounded" required />
