...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try{
        ...
        for (const key in files) {
            ...
            if (Array.isArray(fileData)) {
                ...
                if (file) {
                    ...
                    filePaths[key] = `/uploads/service/${fileName}`;
                }
            } else if (fileData) {
        ...
        try {
            const service = await prisma.ourServices.create({
                data: {
                    serviceTitle: rawFields.serviceTitle as string,
                    description: rawFields.description as string,
                    status: rawFields.status == "true" ? true : false as boolean,
                    showOnHome: rawFields.showOnHome  == "true" ? true : false as boolean,
                    image: filePaths.image as string,
                },
            });
